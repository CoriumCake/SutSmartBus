...
double _perpendicularDistance(
  ...
  return sqrt(
      pow(point.longitude - closestX, 2) + pow(point.latitude - closestY, 2));
}
...
List<({double latitude, double longitude})> simplifyPolyline(
  ...
  if (maxDist > tolerance) {
    final left =
        simplifyPolyline(points.sublist(0, maxIdx + 1), tolerance: tolerance);
    final right =
        simplifyPolyline(points.sublist(maxIdx), tolerance: tolerance);
